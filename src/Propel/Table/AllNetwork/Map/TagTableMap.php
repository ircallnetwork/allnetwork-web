...
use Propel\Table\AllNetwork\Tag;
...
class TagTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Propel.Table.AllNetwork.Map.TagTableMap';
     ...
     */
    const OM_CLASS = '\\Propel\\Table\\AllNetwork\\Tag';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Propel.Table.AllNetwork.Tag';
    ...
    /**
     * the column name for the tag_status field
     */
    const COL_TAG_STATUS = 'tag.tag_status';

    /**
     * the column name for the tag_totalpost field
     */
    const COL_TAG_TOTALPOST = 'tag.tag_totalpost';

    /**
     * the column name for the nb_posts field
     */
    const COL_NB_POSTS = 'tag.nb_posts';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Name', 'Status', 'TagTotalpost', 'NbPosts', ),
        self::TYPE_CAMELNAME     => array('name', 'status', 'tagTotalpost', 'nbPosts', ),
        self::TYPE_COLNAME       => array(TagTableMap::COL_TAG_NAME, TagTableMap::COL_TAG_STATUS, TagTableMap::COL_TAG_TOTALPOST, TagTableMap::COL_NB_POSTS, ),
        self::TYPE_FIELDNAME     => array('tag_name', 'tag_status', 'tag_totalpost', 'nb_posts', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Name' => 0, 'Status' => 1, 'TagTotalpost' => 2, 'NbPosts' => 3, ),
        self::TYPE_CAMELNAME     => array('name' => 0, 'status' => 1, 'tagTotalpost' => 2, 'nbPosts' => 3, ),
        self::TYPE_COLNAME       => array(TagTableMap::COL_TAG_NAME => 0, TagTableMap::COL_TAG_STATUS => 1, TagTableMap::COL_TAG_TOTALPOST => 2, TagTableMap::COL_NB_POSTS => 3, ),
        self::TYPE_FIELDNAME     => array('tag_name' => 0, 'tag_status' => 1, 'tag_totalpost' => 2, 'nb_posts' => 3, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Propel\\Table\\AllNetwork\\Tag');
        $this->setPackage('Propel.Table.AllNetwork');
        $this->setUseIdGenerator(false);
        // columns
        $this->addPrimaryKey('tag_name', 'Name', 'VARCHAR', true, 32, null);
        $this->addColumn('tag_status', 'Status', 'CHAR', false, 1, 'y');
        $this->addColumn('tag_totalpost', 'TagTotalpost', 'INTEGER', true, null, 0);
        $this->addColumn('nb_posts', 'NbPosts', 'INTEGER', false, null, null);
    } // initialize()
  ...
  array (
    ...
    {
        return array(
            'aggregate_column' => array('name' => 'nb_posts', 'expression' => 'COUNT(posttag_post)', 'condition' => '', 'foreign_table' => 'posttag', 'foreign_schema' => '', ),
        );
    } // getBehaviors()
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(TagTableMap::COL_TAG_STATUS);
            $criteria->addSelectColumn(TagTableMap::COL_TAG_TOTALPOST);
            $criteria->addSelectColumn(TagTableMap::COL_NB_POSTS);
        } else {
            $criteria->addSelectColumn($alias . '.tag_name');
            $criteria->addSelectColumn($alias . '.tag_status');
            $criteria->addSelectColumn($alias . '.tag_totalpost');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Propel\Table\AllNetwork\Tag) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(TagTableMap::DATABASE_NAME);
            $criteria->add(TagTableMap::COL_TAG_NAME, (array) $values, Criteria::IN);
        }
